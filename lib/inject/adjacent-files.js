...
module.exports = function (extension, opts, recurse) {
  ...
  return through.obj(function (file, encoding, done) {
    ...
    function srcStream() {
      ...
      var fileBase     = path.resolve((typeof recurse === 'string') ? recurse : file.base);
      var fileRelative = path.relative(fileBase, filePath);
      ...
      var split = recurse ? fileRelative.split(/[\\\/]/g) : [ fileRelative ];
      for(var i = 0, glob = [ ]; i <= split.length; i++) {
        extensions.forEach(function(extension) {
          glob.push([ fileBase ]
            .concat(split.slice(0, i))
            .concat('*.' + extension)
            .join('/'));
        });
      }
      return gulp.src(glob, { read: false })
        .pipe(semiflat(fileBase))
        .pipe(slash());
    }
