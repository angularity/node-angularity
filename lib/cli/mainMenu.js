...
function handleMainMenu(answer) {
  ...
  if (taskName === 'generate project') {
    ...
    installMenu.promptSetupMenu();

  } else if (taskName === 'initialise angularity') {
    generatorUtil.createAngularityProjectConfig(process.cwd());

    console.log('A new angularity configuration file has been created. Please run Angularity again.');
    //todo prompt user for each option like `npm init`. Provide default package.json, bower,
    // verify if source-lib folders exist etc...
  }
}
