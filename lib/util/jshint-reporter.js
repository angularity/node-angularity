...
function checkJsHintReporter(argv) {
  if (argv.help) {
    ...
    var value = argv[yargsOptionDefiniton.key];
    if (typeof value === 'string' && value.length > 0) {
      try {
        getJsHintReporter(value);
      }
      catch (ex) {
        throw new Error('Illegal value for "'+yargsOptionDefiniton.key+'"\n' + ex);
      }
      return true;
    }
    else {
      throw new Error('Required option "'+yargsOptionDefiniton.key+'" in not specified');
    }
  }
}
