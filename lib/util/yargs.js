 ...
 * @param {object} validation A hash of validation methods
 * @return {function} A yargs check function that throws Error on validation fail
 */
function getCheckFor(validation) {
  return function check(argv) {
    for (var key in validation) {
      if (key in argv) {
        var message = validation[key](argv[key]);
