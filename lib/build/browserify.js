...
function compile(bannerWidth, transforms) {
   ...
   * @param {string} prefix Optional prefix for the result
   * @returns {function} A root relative conversion function
   */
  function getRootRelative(prefix) {
    return function (filePath, i, array) {
      var rootRelative = slash(path.relative(process.cwd(), filePath));
      var isProject    = (rootRelative.slice(0, 2) !== '..');
      var result       = [prefix, (isProject ? rootRelative : path.basename(rootRelative))]
        .filter(Boolean)
        .join(path.sep);
  ...
  function bundle(stream, files, bundlename, minify, sourceMapBase, done) {
    ...
    // when we use minification we will get code, source-map
    ...
    bundler.bundle(function (error, codeOrBuffer, map) {
      if (!error) {
        ...
        var anonymous = external.replace(anonymised, '"', '"');   // anonymise module paths
        delete sourceMap.file;
        delete sourceMap.sourcesContent;
        sourceMap.sources
          .forEach(getRootRelative(sourceMapBase));
