 ...
 * @param {boolean} allowProjectRelative Simulates an npm-module for the current project
 * @return {function} A require transform for browserify
 */
function requireTransform(allowProjectRelative) {
  ...
  return transformTools.makeRequireTransform('requireTransform', null, function (args, opts, done) {
    ...
    var firstTerm  = split.splice(0, 1)[0];
    var isRelevant = (firstTerm.length) && !(/^\.{1,2}$/.test(firstTerm));
    if (isRelevant) {
      ...
      if ((packageJson) && (typeof packageJson.name === 'string') && (packageJson.name === firstTerm)) {
        return done(null, 'require("' + process.cwd() + split.join('') + '")');
      ...
      // bower project
        ...
        if (isFound) {
          return done(null, 'require("' + slash(path.resolve(path.join(BOWER, original))) + '")');
        }
      }
    }
    done();
...
function compile(bannerWidth, transforms) {
  ...
  function bundle(stream, files, bundlename, minify, done) {
    ...
    var bundler = browserify({
      debug: true
    }).on('file', function fileHandler(filename) {
        var length = filename.length;
        var zeroPadded = ((new Array(length)).join('0') + String(count++)).slice(-length);
        anonymised.define(zeroPadded, filename);
      })
    ...
    if (isMinify) {
      ...
      bundler.bundle().on('error', errorHandler).pipe(moldSourceMap.transform(function (molder, complete) {
