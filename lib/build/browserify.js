...
/**
 * Test a given espima AST node as a literal with one of the possible given values
...
/**
 * Test a given espima AST node as a method with one of the possible given names
...
module.exports = function (bannerWidth) {
  ...
  var self       = {
    ...
    /**
     * Convert @ in jasmine describe('@'...) and module('@'...) into the full filename
     */
    jasmineTransform: transformTools.makeFalafelTransform('requireTransform', null, function (node, options, done) {
      var isValid = isLiteralAST(node, '@') && isMethodAST(node.parent, 'describe', 'module');
      if (isValid) {
     ...
     * @param {...function} transforms Any number of browserify transforms
     * @returns {stream.Through} A through stream that performs the operation of a gulp stream
     */
    compile: function () {
