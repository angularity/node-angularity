  ...
  var self       = {
    ...
    compile: function () {
      ...
      function groupByFilename(value, i, array) {
        var current      = String(value).split(/\:\d+/).shift();
        var previous     = String(array[i-1]).split(/\:\d+/).shift();
        var isDissimilar = (i > 1) && (i < array.length) && (current !== previous);
      ...
      function bundle(stream, files, bundlename, minifiy, done) {
        ...
        function errorHandler(error) {
          ...
          // Error: <reason> from '<file>'
          } else if (analysis = /^\s*Error\:\s*(.*)\s*from\s*\'(.*)\'\s*$/.exec(text)) {
            message = analysis[2] + ':0:0: ' + analysis[1] + '\n';
        ...
        if (isMinify) {
          ...
          bundler.bundle(function (error, code, map) {
            if (!error) {
              ...
              sourcemap.sources = sourcemap.sources.map(rootRelative);
