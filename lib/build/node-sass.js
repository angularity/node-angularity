...
function encodeRelativeURL(startPath, relative, callerPath) {
  ...
  if (fs.existsSync(absStart) && (absStart !== process.cwd())) {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      var candidates = childDirs.concat(parentDir)
        .filter(function excludeCallerDir(absoluteDir) {
          return (absoluteDir !== callerPath);
        })
        .map(function encode(absoluteDir) {
          return encodeRelativeURL(absoluteDir, relative, absStart);
        })
        .filter(Boolean);
      if (candidates.length > 1) {
        gulp.log('Warning: More than one candidate for URL:' + relative);
      }

      // will be undefined where length is zero
      return candidates[0];
    }
  } else {
    return;
          ...
          visit(stylesheet, function (declarations, node) {
            ...
            declarations.forEach(function (declaration) {
              ...
              declaration.value = declaration.value
                .split(/url\s*\(\s*['"]([^'"?#]*)(?:\?[^'"]*)?['"]\s*\)/g)  // split by url statements
                .map(function (token, i) {
                  return ((i % 2) && encodeRelativeURL(sassDir, token)) || token;
                }).join('');
            });
          });
        }
