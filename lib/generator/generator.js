...
function GeneratorProject(type) {
  ...
  return {
    ...
    commonTemplatesPath: path.join(__dirname, 'templates'),
    ...
    createJSHint: function () {
      var hintFolder = path.join(this.commonTemplatesPath, '.jshintrc');
      cp(hintFolder, this.destination);
    },

    createAngularityProjectConfig: function (override) {
      //todo angularity@1.0.1
      var configTemplatePath = path.join(this.commonTemplatesPath, 'angularity.json');
...
util.createAngularityConfig = function (override) {
  var configTemplatePath = path.join(this.commonTemplatesPath, '.angularity');
  var configTemplate = fs.readFileSync(configTemplatePath);
