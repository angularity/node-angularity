      ...
      return {
        ...
        all: function () {
          ...
          return through.obj(function (file, encoding, done) {
            ...
            var candidate = path.dirname(file.relative);
            if (!base) {
              base = candidate;
            } else if (candidate !== base) {
              throw new Error('All files in the stream must share a common base path.')
            }
            done();
          }, function (done) {
            bundle(this, pending, path.join(basePath, 'test-main.js'), done);
          });
        }
      }
    }
