...
function validateConfig(configData) {
  return (typeof configData.name === 'string') && (typeof configData.version === 'string');
}

function validateAngularityConfig(configData) {
  return (typeof configData.serverHttpPort === 'string') && (typeof configData.javascriptVersion === 'string') &&
    (typeof configData.consoleWidth === 'string');
}
...
function synchroniseConfigs(config) {
  ...
  tainted.forEach(function (config) {
    ...
    var text = JSON.stringify(copy, null, 4);
...
function Config() {
  ...
  this.globalConfig = JSON.parse(fs.readFileSync(CONFIG_PATH_GLOBAL, 'utf8'));
  if (!validateAngularityConfig(this.globalConfig)) {
    throw new Error('There was an issue validating your .angularity file.');
  }
  ...
  if (this.projectConfigPresent) {
    ...
    if (!validateConfig(this.projectConfig)) {
  ...
  if (this.npmConfigPresent) {
    ...
    if (!validateConfig(this.npmConfig)) {
  ...
  if (this.bowerConfigPresent) {
    ...
    if (!validateConfig(this.bowerConfig)) {
