...
function Config() {
  ...
  // Keep the config version numbers in sync between the npm, bower and angularity.
...
//todo use a schema library like... https://github.com/apiaryio/Amanda
  ...
  function synchroniseConfigVersion() {
    if (self.projectConfigPresent && self.npmConfigPresent) {
      ...
      if (self.projectConfig.version !== self.npmConfig.version !== self.bowerConfig.version) {
        var values = [
